...
import { getOutputFormat, getTraceFilePath, sleep } from "./common.js";
...
export default async function startTests(config){
    ...
    await page.goto(
        "file://" + path.resolve(path.join( config.path_to_folder, "empty_page.html")),
        {
            waitUntil: "networkidle2"
        }
    );
    
    // if( task_config.load_libraries !== false ){ //Not possible since the task is not loaded yet.
    ...
    if( failed && !config.keep_tab ) process.exit(1);
}
...
async function runTest( page, config, task_config, candidate, task_path_config ){
    ...
    if( loaded_candidates.indexOf(candidate) == -1 ){
        ...
        if( task_config.load_libraries !== false ){
    ...
    await page.evaluate(
        (browser_task_config, task_name) => {
            ...
            window.browser_task_config.repeats = 1;
