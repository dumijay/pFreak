function showBenchmarkResults(){
    ...
    if( !base_candidate_inited ){
        ...
        var unique_base_candidates = {};
        
        for( var category in category_results ){
            var category = category_results[category];

            for( var candidate in category.candidate_wise ){
                if( !unique_base_candidates[candidate] ){

                    unique_base_candidates[candidate] = true;
                    candidate_wise_array.push( {
                        name: candidate
                        // duration: category.candidate_wise[candidate]  
                    });
                ...
                value_array = this_task[candidate].map(
                    function(iteration){
                        ...
                        for( var kpi in iteration ){
                            ...
                            if( selected_kpis.length == 0 || selected_kpis.indexOf(processed_kpi) != -1 ) duration += iteration[kpi];
                        }
