...
import { log, EMPTY_PAGE_PATH, getOutputFormat, getSystemInformation, getTraceFilePath, sleep } from "./common.js";
    ...
    for( var task_i in task_path_configs ){
        ...
        for( var iteration_i = 0; iteration_i < iterations_count; iteration_i++ ){
            ...
            for( var candidate_i = 0; candidate_i < task_config.candidates.length; candidate_i++ ){
                ...
                var task_duration = await runTask( config, task_config, candidate, task_path_config, iteration_i )
                    .catch( error => {
                        console.trace(error);
...
async function runTask( config, task_config, candidate, task_path_config, iteration_i ){
    ...
    await client.send('Emulation.setCPUThrottlingRate', { rate: task_config.cpu_slowdown || config.cpu_slowdown || 4 }); //Increase below sleep time if throttled more
    ...
    await page.evaluate(
        (browser_task_config, task_name) => {
            ...
            setTimeout( runCandidate, 1500 );
        },
        task_config.browser_task_config, 
        task_config.short_name
    );

    const raw_trace_file_path = path.join( config.path_to_folder, `output/traces/${task_config.category}_${task_config.short_name}_${candidate}_${iteration_i}.json` );
    ...
    sleep_time += 2000; //For the runCandidate setTimeout with a margin

    sleep_time = sleep_time / 4 * config.cpu_slowdown;
