function showSlownessTable(title, results, show_trace_data, category){
    ...
    for( var task in results.task_candidate_wise ){
        ...
        for( var candidate in results.candidate_wise ){
            ...
            if( typeof task_candidates[candidate] != 'undefined' ){
                ...
                if( task_candidates[base_candidate] ){
                    avg = Math.round( (task_candidates[candidate].mean / task_candidates[base_candidate].mean) * 10 ) / 10;
                }
    ...
    for( var candidate in results.candidate_wise ){
        var candidate_result, base_candidate_result;

        if( Array.isArray(results.candidate_wise[candidate]) ){
            candidate_result = results.candidate_wise[candidate][0];
            base_candidate_result = results.candidate_wise[base_candidate][0];
        }
        else{
            candidate_result = results.candidate_wise[candidate];
            base_candidate_result = results.candidate_wise[base_candidate];
        }

        var avg = Math.round( (candidate_result.mean / base_candidate_result.mean) * 10 ) / 10;
